...
class Command extends Base {
    ...
    constructor() {
        ...
        this.nombre = 'double'
        this.description = 'Pide dos cartas durante tu turno en el blackjack.'
        this.uses = [
            ['Pedir dos cartas', ''],
        ]
        ...
        this.exec = async (message, args, server) => {
            ...
            const userCard = [ game.random(), game.random() ]
            const botCard = [ game.random() ]

            if(Math.random() > 0.6) botCard.push(game.random())

            data.user.points += (userCard[0] + userCard[1])
            data.botPoints += (botCard[0] + (botCard.length > 1 ? botCard[1] : 0))
            data.gameTurn++
            ...
            const embed = {
                ...
                fields: [
                    { name: message.author.tag, value: '```Ha obtenido ' + userCard.join(" y ") + '\nPuntos: ' + data.user.points + '/21```' },
                    { name: 'Contrincante', value: '```Ha obtenido ' + botCard.join(" y ") + '\nPuntos: ' + data.botPoints + '/21```' },
                ],
