...
class Command extends Base {
    ...
    constructor() {
        ...
        this.alias = ['sendgems']
        this.cooldown = 4
        this.description = 'Envía gemas a otro usuario de una forma más rápida.'
        this.uses = [
            ['Enviar gemas', '<@usuario> <cantidad>'],
        ]
        ...
        this.exec = async (message, args, server) => {
            ...
            if(balances.user.gems < amount) return message.reply('no cuentas con esta cantidad de dinero.')

            await this.db.balance.increment({ gems: -amount }, { where: { id: balances.user.id } })
            await this.db.balance.increment({ gems: +amount }, { where: { id: balances.other.id } })

            const embed = {
                color: '#16A167',
                author: [message.author.tag, message.author.avatarURL()],
                description: 'Has enviado ' + amount + ' gemas a ' + mencion.user.tag + '.',
                timestamp: true,
            }
            ...
            embed.fields = [
                ...
                { name: 'Monto', value: amount + ' Gemas', inline: true },
